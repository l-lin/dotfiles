...
local function scrape(url_parser)
  ...
  local response = require("functions.http_client").new():get(url_parser.input_url)
...
local function paste_url()
  ...
  local success, url_parser = pcall(require("functions.url_parser").new, input)
  if success then
    input = create_markdown_link(url_parser)
  end

  require("functions.insert_text").at_current_cursor(input)
end
