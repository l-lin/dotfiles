...
local function is_url(input_url)
	if input_url == nil or input_url == "" then
		return false
	end
...
local function http_get(input_url)
	...
	local curl_cmd = string.format('curl -s -L -H "User-Agent: Mozilla/5.0 (compatible; TitleExtractor/1.0)" "%s"', input_url)
